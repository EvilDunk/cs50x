...
function Player:init(map)
    ...
    self.sounds['jump']:setVolume(0.15)
    self.sounds['coin']:setVolume(0.15)
    self.sounds['hit']:setVolume(0.15)
...
function Player:checkVictory()
    if self.victory == true then
        return true
    end
end

function Player:checkFlagTouch()
    if self.map:tileAt(self.x + self.width, self.y).id == POLE_BOTTOM or
        self.map:tileAt(self.x + self.width, self.y + self.height - 1).id == POLE_BOTTOM then
        self.victory = true
    end
end
